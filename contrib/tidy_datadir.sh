...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete DACash database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no DACash datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old DACash datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected DACash 0.7 datadir."
    ;;
  3)
    echo "Detected DACash pre-0.8 datadir."
    ;;
  4)
    echo "Detected DACash 0.8 datadir."
    ;;
esac
