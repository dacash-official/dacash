...
const char * const BITCOIN_CONF_FILENAME = "dacash.conf";
const char * const BITCOIN_PID_FILENAME = "dacashd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "dacash" is a composite category enabling all DACash-related debug output
            if(ptrCategory->count(string("dacash"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DACashCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DACashCore
    // Mac: ~/Library/Application Support/DACashCore
    // Unix: ~/.dacashcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DACashCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DACashCore";
#else
    // Unix
    return pathRet / ".dacashcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty dacash.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override dacash.conf
